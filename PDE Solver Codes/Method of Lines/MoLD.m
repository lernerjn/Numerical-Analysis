...
%Dirichlet boundary conditions u(t,0)=u(t,1)=0. This generates a stiff ODE,
...
exactSolnFun = @(t,x)exp((-pi^2)*(t))*sin(pi*x);
exactSoln = zeros(n,1);
for i = 2:n-1
...
plot3(t, mesh2, est);
grid on;
title('Solution to Heat Equation (with Dirichlet BCs) Found by Method of Lines');
xlabel('t');
ylabel('x');
zlabel('u(x,t)');
