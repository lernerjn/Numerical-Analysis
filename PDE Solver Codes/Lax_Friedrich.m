...
% explicit Lax-Friedrich scheme for numerically estimating solutions to PDEs
% Input:
...
for i = 1:timeSteps 
    ...
    u_new(1,1) = -(a*deltaT/(2*deltaX))*(u(2,1)-(u(N+1,1))) + (1/2)*(u(2,1)+(u(N+1,1)));
    u_new(N+1,1) = -(a*deltaT/(2*deltaX))*((u(1,1))-u(N,1)) + (1/2)*(u(1,1)+u(N,1));
    
    %Iterate over space, determine each value based on values from the
    %previous time step
    for k = 2:N
        u_new(k,1) = -(a*deltaT/(2*deltaX))*(u(k+1,1)-u(k-1,1)) + (1/2)*(u(k+1,1)+u(k-1,1));
    end
    u = u_new;
end
